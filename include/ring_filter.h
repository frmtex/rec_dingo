...
public:
    ...
    //      Independently of these, a small built-in band (see remove_stripes' definition) around
    //      the rotation axis is kept out of the wavelet filter: the same "can't tell a stripe from
    //      real content" problem is at its worst exactly at r=0 (a ring of vanishing radius
    //      carries no distinguishing signal at all), and the residual, imperfectly-damped
    //      angular-frequency content this filter can leave there was observed backprojecting into
    //      a starburst artifact. Rather than leaving those columns untouched (which leaves any
    //      detector defect in them as a small dot at the axis), only their angle-constant offset
    //      relative to a line interpolated from the neighboring columns is removed - see the end of
    //      remove_stripes. This band is combined with (not replaced by) any wider mask passed here.
